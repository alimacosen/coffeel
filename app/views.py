...
from .forms import LoginForm, RegisterForm, AddGoodsForm, CateForm, BrandForm, \
    ...
    InventoryForm, EditGoodsForm, EditCoverForm, EditImageForm, EditCustForm, EditPasswordForm
...
def random_filename(filename):
    ext = os.path.splitext(filename)[1]  # 默认返回(fname,fextension), 这里取后缀
    new_filename = uuid.uuid4().hex + ext  # uuid4:伪随机数 hex:32个16进制数
    return new_filename


@app.before_request
def before_request():
    # g(global) 存储用户信息 不可跨request
    g.user = current_user
